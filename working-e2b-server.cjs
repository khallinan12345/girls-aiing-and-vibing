...
`;
      ...
      // Extract output using the SAME logic as the working debug server
      let output = '';
      
      if (result && result.logs && result.logs.stdout && result.logs.stdout.length > 0) {
        output = result.logs.stdout.join('').trim();
        console.log(`✅ Found output: "${output}"`);
      } else if (result && result.logs && result.logs.stderr && result.logs.stderr.length > 0) {
        output = 'Error: ' + result.logs.stderr.join('').trim();
        console.log(`⚠️  Found error: "${output}"`);
      } else {
        output = 'Code executed successfully (no output)';
        console.log(`❌ No output found`);
        // Debug what we actually got
        console.log('Debug - result:', result ? 'exists' : 'null');
        console.log('Debug - result.logs:', result ? (result.logs ? 'exists' : 'missing') : 'N/A');
        console.log('Debug - result.logs.stdout:', result && result.logs ? (result.logs.stdout ? `array length ${result.logs.stdout.length}` : 'missing') : 'N/A');
      }
